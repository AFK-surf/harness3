    ...
    case message {
      llm.Message(llm.Assistant, content) ->
        list.any(content, fn(part) {
          case part {
            llm.ToolCall(_, name, _) -> name == tool_name
            _ -> False
          }
        })
      _ -> False
...
pub fn inject_rpc_forwards_to_awake_host_and_queues_tool_call_test() {
  let root = temporary_root()
  let backend = local.new(local.config(root))
  use <- exception.defer(fn() {
    let _ = remove_directory(root)
  ...
  let config =
    agent_group.Config(backend, "groups/rpc-inject", [profile], 10, 100)
  let assert Ok(_) =
    agent_group.create(
      config,
  ...
  let assert Ok(_blocked) = process.receive(started, within: 2000)
  assert list.contains(agent_group_registry.alive_ids(), "rpc-inject-group")

  // The request enters through a different node and is forwarded to the
  // group owner; the running agent queues the synthetic pair as pending.
  let assert Ok("ok") =
    core.call(
      entry_ip,
      entry_port,
      core.token(entry),
      agent_group_rpc.inject_method_name(),
      agent_group_rpc.inject_request(
        "rpc-inject-group",
        "agent",
  ...
  assert synthetic_call_pending(
    queued_agent.pending_messages,
    "team.receive_message",
  )
  assert !synthetic_call_pending(queued_agent.messages, "team.receive_message")
  ...
  assert !list.contains(agent_group_registry.alive_ids(), "rpc-inject-group")
  let assert Error(core.RemoteResponse(500, not_running)) =
    core.call(
      entry_ip,
      entry_port,
      core.token(entry),
  ...
  assert string.contains(string.inspect(not_running), "not_running")
  let assert Ok(snapshot) = agent_group.load(config)
  let assert agent_group.Idle(released_epoch) = snapshot.execution
  assert released_epoch > 0
}
